...
- [ ] **[God Class] Phase 0 — Baseline: create refactor branch and confirm clean state** *(Owner: Arjun Sen)*
  Create branch `refactor/mainwindow-decomposition` from `main`. Confirm solution builds without
  errors and all existing tests pass. Record baseline line count and field count for MainWindow so
  progress can be measured at each phase. No code changes in this step — setup only.
  Tag: God Class

- [ ] **[God Class] Phase 1A — Extract WorkspaceFileWatcherCoordinator** *(Owner: Arjun Sen)*
  Pull the 5+ `FileSystemWatcher` instances and their event handlers out of MainWindow into a new
  `WorkspaceFileWatcherCoordinator` class. MainWindow holds a reference and delegates watcher setup
  to the coordinator. No XAML changes required. Ship with Vesper unit tests.
  Tag: God Class

- [ ] **[God Class] Phase 1B — Extract UiTimingConstants** *(Owner: Arjun Sen)*
  Centralize the 20+ magic delay values scattered through MainWindow (50 ms, 100 ms, 220 ms, etc.)
  into a static `UiTimingConstants` class. Replace all inline numeric literals with named constants.
  No behaviour change — pure refactor. Ship with a test that asserts constant values haven't drifted.
  Tag: God Class

- [ ] **[God Class] Phase 1C — Extract SettingsSnapshotManager** *(Owner: Arjun Sen)*
  Isolate the `_settingsSnapshot` state and all logic that reads/writes it into a new
  `SettingsSnapshotManager` class. MainWindow delegates snapshot operations through the manager.
  No XAML changes required. Ship with Vesper unit tests covering snapshot apply/restore paths.
  Tag: God Class

- [ ] **[God Class] Phase 2A — Extract ScreenshotService** *(Owner: Arjun Sen)*
  The screenshot capture logic (~200 lines) is relatively self-contained inside MainWindow.
  Move it into a `ScreenshotService` class with a clean interface. MainWindow calls the service.
  Depends on: Phase 1 complete. Ship with unit tests.
  Tag: God Class

- [ ] **[God Class] Phase 2B — Extract GuidedTourCoordinator** *(Owner: Arjun Sen)*
  The 15+ tour-related fields and their hidden state machine currently live in MainWindow.
  Extract into `GuidedTourCoordinator`. MainWindow wires the coordinator into the UI event chain
  but owns no tour state directly. Depends on: Phase 1 complete. Ship with unit tests.
  Tag: God Class

- [ ] **[God Class] Phase 2C — Extract PromptQueueCoordinator** *(Owner: Arjun Sen)*
  Begin thinning the 380+ direct event subscriptions by extracting queue-related subscriptions into
  `PromptQueueCoordinator`. This is the foundation for the Phase 5 event broker. Depends on: Phase 1
  complete. Ship with Vesper unit tests covering event wiring.
  Tag: God Class

- [ ] **[God Class] Phase 3A — Extract TranscriptPanelController** *(Owner: Lyra Morn)*
  Extract all logic that drives `ITranscriptRenderSink` out of MainWindow into
  `TranscriptPanelController`. MainWindow holds the controller and routes relevant events to it.
  XAML touch required for binding adjustments. Depends on: Phase 2 complete. Ship with unit tests.
  Tag: God Class

- [ ] **[God Class] Phase 3B — Extract AgentRosterController** *(Owner: Lyra Morn)*
  Extract all logic that drives `IAgentRosterView` out of MainWindow into `AgentRosterController`.
  MainWindow holds the controller and delegates roster updates through it. XAML touch required.
  Depends on: Phase 2 complete. Ship with unit tests.
  Tag: God Class

- [ ] **[God Class] Phase 3C — Extract InboxPanelController** *(Owner: Lyra Morn)*
  Extract inbox-panel driving logic out of MainWindow into `InboxPanelController`. MainWindow
  delegates all inbox state changes through the controller. XAML touch required.
  Depends on: Phase 2 complete. Ship with unit tests.
  Tag: God Class

- [ ] **[God Class] Phase 4 — Interface segregation: MainWindow implements no interfaces directly** *(Owner: Arjun Sen)*
  After Phase 3, MainWindow should implement none of its 6 interfaces directly. Each interface
  should be satisfied by the relevant controller extracted in Phase 3. Wire MainWindow → controller
  → interface. Depends on: Phase 3 complete. Verify with a build that MainWindow has no direct
  `IXxx` implementations.
  Tag: God Class

- [ ] **[God Class] Phase 5 — Event broker: replace direct subscriptions with pub-sub** *(Owner: Arjun Sen)*
  Replace the remaining 380+ direct event subscriptions in MainWindow with a lightweight pub-sub
  event broker. Eliminates memory-leak risk from undisposed handlers and decouples senders from
  receivers. Depends on: Phase 4 complete (all controllers in place as natural subscriber boundaries).
  Ship with Vesper unit tests asserting correct event delivery and no leaked references.
  Tag: God Class
