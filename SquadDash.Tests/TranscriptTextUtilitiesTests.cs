...
internal sealed class TranscriptTextUtilitiesTests {
    ...
    [Test]
    public void SanitizeResponseText_PlanRevision_StripsPayloadAndKeepsJsonInspectable()
    {
        const string text = "Revision proposed.\n\nPLAN_REVISION_JSON:\n" +
            "{ \"planId\": \"PLAN-1\", \"baseRevision\": \"rev-1\", \"summary\": \"Change step 7\", " +
            "\"revisedPlan\": { \"groupId\": \"PLAN-1\" } }";

        var blocks = TranscriptTextUtilities.ExtractInspectableProtocolJsonBlocks(text);

        Assert.Multiple(() =>
        {
            Assert.That(TranscriptTextUtilities.SanitizeResponseText(text), Is.EqualTo("Revision proposed."));
            Assert.That(blocks, Has.Count.EqualTo(1));
            Assert.That(blocks[0].Marker, Is.EqualTo("PLAN_REVISION_JSON"));
            Assert.That(blocks[0].Json, Does.Contain("Change step 7"));
        });
    }
