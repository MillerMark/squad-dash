...
> **Note:** The Loop Panel drives SquadDash's own *native loop* mechanism (agents run inside SquadDash) or the `squad` CLI's loop mode, depending on the mode radio selected. These are two distinct execution paths.
