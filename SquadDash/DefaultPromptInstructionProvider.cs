...
internal sealed class DefaultPromptInstructionProvider : IPromptInstructionProvider {
    ...
    private static readonly PromptInstructionSet Instance = new(
        ...
        InboxMessage:
            ...
            "</inbox_instructions>",
        QuickReply:
            "Quick replies may include an optional `tone` field. Omit it for normal actions. " +
            "Use `\"warning\"` only when the action has a meaningful, non-destructive risk the user should understand before proceeding, and explain that risk in `reason`. " +
            "Use `\"destructive\"` only for an action that discards, ends, or irreversibly changes work. Never use tone merely to attract attention.\n\n" +
